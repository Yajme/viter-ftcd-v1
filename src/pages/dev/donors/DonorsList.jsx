...
export default function DonorsList({ setItemEdit, itemEdit }) {
  ...
  const {
    ...
    queryKey: ["donors", searchValue, store.isSearch, filterData],
    queryFn: async ({ pageParam = 1 }) =>
      await queryDataInfinite(
        `${apiVersion}/controllers/dev/donors/page.php?start=${pageParam}`,
        `${apiVersion}/controllers/dev/donors/page.php?start=${pageParam}`, // list endpoint
...
//   }, [inView]);
  ...
  return (
    <>
                        ...
                        <div className="flex items-center gap-3 justify-center">
                          {item.donor_is_active == 1 ? (
                            <>
                              ...
                              <button className="btn-action-table bg-primary text-white rounded-md px-6 py-0.5">
                              Donate
                              </button>
                            </>
                          ) : (
      ...
      {store.isRestore && (
        <ModalRestore
          mysqlApiRestore={`${apiVersion}/controllers/dev/donors/index.php?id=${itemEdit.donor_aid}&action=archive`}
          dataItem={itemEdit}
          queryKey={"donors"}
      ...
      {store.isDelete && (
        <ModalDelete
          mysqlApiDelete={`${apiVersion}/controllers/dev/donors/index.php?id=${itemEdit.donor_aid}`}
          dataItem={itemEdit}
          queryKey={"donors"}
      ...
      {store.isArchive && (
        <ModalArchive
          mysqlApiArchive={`${apiVersion}/controllers/dev/donors/index.php?id=${itemEdit.donor_aid}&action=archive`}
          dataItem={itemEdit}
          queryKey={"donors"}
